...
export default function InputText({w,icon,password,placeholder,iconSize,fontSize,inputMode,error=false,errorMessage,borderColor,inputValue,setInputValue,backgroundColor,borderRadius}) {
   ...
   const [errorMsg,setErrorMsg]=useState(errorMessage)
    ...
    return(<View style={{ margin:5}}>
      <View style={{ flexDirection:'row',  borderColor:error?'#eb2f06':borderColor,borderWidth:1,borderRadius:borderRadius, width:width*w,height:height*0.07,alignItems:'center',backgroundColor:backgroundColor}}>
       ...
       <TextInput secureTextEntry={password} onFocus={()=>{}} onChangeText={(v)=>{setInputValue(v)}} inputMode={inputMode} placeholder={placeholder} style={{fontSize:fontSize?fontSize:20}} />
      </View>
